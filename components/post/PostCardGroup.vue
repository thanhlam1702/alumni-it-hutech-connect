  ...
  <div class="posts">
    <div v-for="(item, ipost) in posts" :key="item._id" class="posts__item">
      <div class="posts__item-user">
        ...
        <div class="name">
          <nuxt-link :to="'/user/' + item.owner">
            {{ item.fullName ? item.owner.fullName : item.name }}
          </nuxt-link>
        </div>
      ...
      <div class="posts__item-control">
        ...
        <div class="comment">
          <nuxt-link :to="'/groups/post/' + item._id">Bình luận</nuxt-link>
        </div>
      </div>
    </div>
  </div>
  ...
  watch: {
    flagModal() {
      ...
      document
        ...
        .addEventListener('click', function () {
          _this.$router.push(`/groups/post/${_this.idPost}`)
          _this.closeModal()
        })
      document
    ...
    async fetchLike(id, pos) {
      try {
        const result = await this.$axios.$get(
          process.env.baseApiUrl + '/groups/deck/' + id
        )
    ...
    async onLike(id, pos) {
      try {
        const result = await this.$axios.$post(
          process.env.baseApiUrl + '/decks/group/' + id
        )
        if (result.success) {
